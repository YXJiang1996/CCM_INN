...
def train(model, train_loader, n_its_per_epoch, zeros_noise_scale, batch_size, ndim_tot, ndim_x, ndim_y, ndim_z,
    ...
    for x, y in train_loader:
        ...
        with torch.no_grad():
            if (i_epoch % 20 == 0) & (i_epoch > 0) & batch_idx == 1:
                r = np.logspace(np.log10(0.1), np.log10(50), num=200)
                mmdValsFor = []
                mmdValsBack = []

                mmdFns = []
                for a in r:
                    mm = MMD_multiscale_on(alphas=[float(a)])
                    mmdFns.append(mm)

                for mm in mmdFns:
                    mmdValsFor.append(mm(output_block_grad, y_short).item())
                    mmdValsBack.append(mm(output_rev_rand[:, :ndim_x], x[:, :ndim_x]).item())
                ...
                if not idxFor is None:
                    alpha_for.append(float(r[idxFor]))
                if not idxBack is None:
                    alpha_rev.append(float(r[idxBack]))

    alphaFor = alphaFor if alpha_for.__len__() == 0 else alpha_for
    alphaBack = alphaBack if alpha_rev.__len__() == 0 else alpha_rev
    return l_tot / batch_idx, l, l_rev, alphaFor, alphaBack
...
def main():
    ...
    optical_model = 'four_flux'  # the optical model to use
    ...
    alpha_for = [8, 11]
    alpha_rev = [1.4, 2, 5.5, 7]
    loss_backward = MMD_multiscale_on(alpha_rev)
    loss_latent = MMD_multiscale_on(alpha_for)
    loss_fit = fit
    ...
    try:
        ...
        for i_epoch in tqdm(range(n_epochs), ascii=True, ncols=80):
            ...
            # train the model
            avg_loss, loss_for, loss_rev, alpha_for, alpha_rev = train(model, train_loader, n_its_per_epoch,
                                                                       zeros_noise_scale, batch_size,
                                                                       ndim_tot, ndim_x, ndim_y, ndim_z, y_noise_scale,
                                                                       optimizer, lambd_predict, loss_fit, lambd_latent,
                                                                       loss_latent, lambd_rev, loss_backward,
                                                                       alpha_for, alpha_rev, i_epoch)
            loss_latent = MMD_multiscale_on(alpha_for)
            loss_backward = MMD_multiscale_on(alpha_rev)
        ...
        for i in range(r):
            for j in range(r):
                ...
                rev_x = model(y_samps, rev=True)[:, :2]
