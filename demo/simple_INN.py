...
class DenseLayer:
    ...
    def _get_transform(self, x):
        ...
        with tf.variable_scope(self.unique_name, reuse=tf.AUTO_REUSE):
            h1 = slim.fully_connected(x, input_dim + 5,
                                      activation_fn=tf.nn.elu)  # +5 just to show that these can be any size
            h2 = slim.fully_connected(h1, input_dim + 5,
                                      activation_fn=tf.nn.elu)  # +5 just to show that these can be any size
...
for i in range(10):
    ...
    # print(z)
    # assert np.allclose(x, net_input_recovered)
    print(diff(x, net_input_recovered))
    print(i)
