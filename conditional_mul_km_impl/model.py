...
def main():
    ...
    Ngrid = 5
    cnt = 0
    lik = np.zeros((r, r, Ngrid ** colors.size))
    for i in range(r):
        for j in range(r):
            mvec, cons, temp = data.get_lik(np.array(r_test[cnt, :]).flatten(), n_grid=Ngrid, model=optical_model,
                                            bound=bound)
    ...
    try:
        ...
        color_names = ['Black', 'Yellow', 'Pink', 'Purple', 'Blue', 'Gray']
        ...
        for i_epoch in tqdm(range(n_epochs), ascii=True, ncols=80):
            ...
            for i in range(r):
                for j in range(r):
                    ...
                    x_norm = pd.DataFrame(
                        (plot_x - plot_x.min(0)) / (plot_x.max(0) - plot_x.min(0)),
                        columns=color_names)
                    ...
                    '''
                    contour_norm = pd.DataFrame((cons - cons.min()) / (cons.max() - cons.min()))
                    contour_trans = pd.concat(
                        [contour_norm[color_names],
                         pd.DataFrame(np.zeros(Ngrid ** colors.size).reshape(Ngrid ** colors.size, 1),
