...
def train(model, train_loader, info, n_its_per_epoch, zeros_noise_scale, batch_size, ndim_tot, ndim_x, ndim_y, ndim_z,
    ...
    for x, y in train_loader:
        ...
        output_rev_rand_grad = torch.cat((output_rev_rand[:, :x_dim],
                                          output_rev_rand[:, x_dim:].data), dim=1)
        x_grad = torch.cat((x[:, :x_dim],
                                          x[:, x_dim:].data), dim=1)
        l_rev = (
                lambd_rev
                * loss_factor
                # * (loss_backward(output_rev_rand[:, :x_dim], x[:, :x_dim])
                * (loss_backward(output_rev_rand_grad, x_grad)
                   )
        )

        l_rev += (0.50 * lambd_predict * loss_fit(output_rev, x)
                  + loss_factor * non_nagative_attachment(10, 2, output_rev[:, :x_dim]).sum())
...
def main():
    ...
    # generate data
    concentrations, reflectance, x, info = data2.generate(
        model=optical_model,
    ...
    model = torch.load('model_dir/km_impl_model_new')
    ...
    try:
        ...
        for i_epoch in tqdm(range(n_epochs), ascii=True, ncols=80):
            ...
            if (i_epoch % 200 == 0) & (i_epoch > 0):
                torch.save(model, 'model_dir/km_impl_model_new2')
            ...
            for i in range(r):
                for j in range(r):
                    ...
                    class_norm = pd.DataFrame(np.concatenate((classes, np.array([[64]]).reshape(1, 1)), axis=0),
                                              columns=['class'])
                    ...
                    for recipe_class in recipe_classes:
                        axes[i, j].scatter(data_plot[class_data == recipe_class][0],
                                           ...
                                           s=2, alpha=0.5)
                    axes[i, j].scatter(data_plot[class_data == 64][0], data_plot[class_data == 64][1], marker='+',
                                       s=30)
        ...
        test_samps = np.array([[0.2869235, 0.3562616, 0.3648288, 0.3690848, 0.3720965, 0.3743734, 0.3748034, 0.3726986,
                                0.3670701, 0.3595550, 0.3495962, 0.3348914, 0.3152257, 0.2898063, 0.2588453, 0.2237525,
                                0.1902168, 0.1687493, 0.1594849, 0.1573166, 0.1543398, 0.1508693, 0.1542613, 0.1652469,
                                0.1817535, 0.1943915, 0.2001948, 0.1974784, 0.1753394, 0.1711839, 0.1716521],
                               [0.2673378, 0.3132285, 0.3183329, 0.3234908, 0.3318701, 0.3409707, 0.3604081, 0.4168356,
                               ...
                               [0.2335778, 0.2669207, 0.2692738, 0.2711587, 0.2714446, 0.2716927, 0.2717375, 0.2719162,
        ...
        test_cons = np.array(
            [[0.127, 0, 0, 0.56, 0, 0.0748], [0, 0.8014, 0.1491, 0, 0.2241, 0], [0.3306, 0, 0.0615, 0.1219, 0, 0]])
        for cnt in range(test_samps.shape[0]):
            ...
            for recipe_class in recipe_classes:
                axes.scatter(data_plot[class_data == recipe_class][0], data_plot[class_data == recipe_class][1],
                             s=2, alpha=0.5)
            axes.scatter(data_plot[class_data == 64][0], data_plot[class_data == 64][1], marker='+', s=30)
            ...
            for j in range(r):
                ...
                for recipe_class in recipe_classes:
                    axes.scatter(data_plot[class_data == recipe_class][0],
                                 data_plot[class_data == recipe_class][1],
                                 s=2, alpha=0.5)
                axes.scatter(data_plot[class_data == 64][0], data_plot[class_data == 64][1], marker='+',
                             s=30)
