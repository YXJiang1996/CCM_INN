...
def main():
    ...
    try:
        ...
        for i_epoch in tqdm(range(n_epochs), ascii=True, ncols=80):
            ...
            for i in range(r):
                for j in range(r):
                    ...
                    axes[i, j].clear()
                    '''
                    axes[i, j].scatter(data_plot[class_data == 1][0], data_plot[class_data == 1][1], label='predicted',
                                       c='blue', s=2, alpha=0.5)
                    axes[i, j].scatter(data_plot[class_data == 0][0], data_plot[class_data == 0][1], label='true',
                                       c='red', marker='+', s=10)
                    '''
        ...
        for cnt in range(test_samps.shape[0]):
            ...
            recipe_ref = data2.recipe_reflectance(test_rev, optical_model)
            print("######## Test Sample %d ########" % cnt)
            for n in range(test_rev.shape[0]):
                print(test_rev[n, :])
                diff = data2.color_diff(test_samps[cnt, :], recipe_ref[n, :])
                print("color diff: %.2f \n" % diff)
            print("\n\n")
        ...
        for i in range(r):
            for j in range(r):
                ...
                recipe_ref = data2.recipe_reflectance(rev_x, optical_model)
                print("######## Test %d ########" % cnt)
                print(c_test[cnt])
                print("################")
                for n in range(rev_x.shape[0]):
                    print(rev_x[n, :])
                    diff = data2.color_diff(r_test[cnt].numpy(), recipe_ref[n, :])
                    print("color diff: %.2f \n" % diff)
                print("\n\n")
