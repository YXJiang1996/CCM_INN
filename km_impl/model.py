...
def train(model, train_loader, n_its_per_epoch, zeros_noise_scale, batch_size, ndim_tot, ndim_x, ndim_y, ndim_z,
    ...
    for x, y in train_loader:
        ...
        # diff = np.array([data.color_diff_tensor(output[i, -ndim_y:], y[i, -ndim_y:]) for i in np.arange(0, x.shape[0], 1)])
        # l = 0.01 * lambd_predict * diff.sum()
...
def main():
    ...
    try:
        ...
        for i_epoch in tqdm(range(n_epochs), ascii=True, ncols=80):
            ...
            # train the model
            avg_loss, loss_for, loss_rev = train(model, train_loader, n_its_per_epoch, zeros_noise_scale, batch_size,
