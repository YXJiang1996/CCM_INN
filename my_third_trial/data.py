...
'''
# 黑07H、黄08、亮蓝20A-2、深蓝2704、浅灰2804、玫红16
ingredients0 = np.array([
    [0.2072717, 0.2314336, 0.2323573, 0.2326192, 0.2315759, 0.2308346, 0.2299640, 0.2295049, 0.2283248, 0.2267959,
     0.2265563, 0.2253986, 0.2243909, 0.2231159, 0.2221052, 0.2208150, 0.2201522, 0.2193270, 0.2182626, 0.2170505,
     0.2162608, 0.2146650, 0.2133304, 0.2119302, 0.2106560, 0.2094081, 0.2085703, 0.2077338, 0.2034682, 0.2023278,
    ...
    [0.3737892, 0.5056624, 0.5303422, 0.5383733, 0.5443333, 0.5562689, 0.5746784, 0.5999998, 0.6406719, 0.6941598,
     ...
     0.9465482, 0.9460330, 0.9458108, 0.9456140, 0.9452482, 0.9455251, 0.9448371, 0.9429901, 0.9404516, 0.9368277,
     0.9308580]])
initial_concentration0 = np.array([0.51, 0.51, 0.497, 0.51, 0.504, 0.503, 0.6, 0.498, 0.508, 0.54])
...
def recipe_reflectance(recipes, model='km'):
    ...
    init_conc_array = np.repeat(initial_concentration0.reshape(10, 1), 31).reshape(10, 31)
    reflectance = np.zeros(31 * recipes.shape[0]).reshape(31, recipes.shape[0])

    if model == 'km':
        fsb = (np.ones_like(background0) - background0) ** 2 / (background0 * 2)
        fst = ((np.ones_like(ingredients0) - ingredients0) ** 2 / (ingredients0 * 2) - fsb) / init_conc_array
        ...
        ones_background = np.ones_like(background0)
        fsb = (8 * background0 + (ones_background - 6 * background0) *
               ((4 * background0 ** 2 - 4 * background0 + 25 * ones_background) ** 0.5)
               + 12 * background0 ** 2 + 5 * ones_background) / (48 * background0)

        ones_ingredients = np.ones_like(ingredients0).reshape(ingredients0.shape[0], ingredients0.shape[1])
        fst = ((8 * ingredients0 + (ones_ingredients - 6 * ingredients0) *
                ((4 * ingredients0 ** 2 - 4 * ingredients0 + 25 * ones_ingredients) ** 0.5)
                + 12 * ingredients0 ** 2 + 5 * ones_ingredients) / (48 * ingredients0) - fsb) / init_conc_array
